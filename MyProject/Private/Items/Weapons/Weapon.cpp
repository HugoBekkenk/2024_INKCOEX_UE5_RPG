...
void AWeapon::AttachMeshToSocket(USceneComponent* InParent, const FName& InSocketName)
{
	...
	ObjectMesh->AttachToComponent(InParent, TransformRules, InSocketName);
}
...
void AWeapon::OnBoxOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	//if (ActorIsSameType(OtherActor, FName("EngageableTarget"))) return;
	...
	if (BoxHit.GetActor())
	{
		...
		//if (ActorIsSameType(BoxHit.GetActor(), FName("EngageableTarget"))) return;
		UGameplayStatics::ApplyDamage(BoxHit.GetActor(), Damage, GetInstigator()->GetController(), this, UDamageType::StaticClass());
		ExecuteGetHit(BoxHit);
		if (!OtherActor->ActorHasTag(FName("Enemy")) && !OtherActor->ActorHasTag(FName("EngageableTarget")))
		{
			CreateFields(BoxHit.ImpactPoint);
		}
		ABaseCharacter* OwnerWeapon = Cast<ABaseCharacter>(GetOwner());
		if (OwnerWeapon)
		{
			ABaseCharacter* CombatCharacter = Cast<ABaseCharacter>(BoxHit.GetActor());
			if (CombatCharacter && CombatCharacter->IsAlive())
			{
				OwnerWeapon->CombatTarget = BoxHit.GetActor();
			}
		}
	}
}
...
void AWeapon::ExecuteGetHit(FHitResult& BoxHit)
{
	...
	if (HitInterface)
	{
		// Use Execute_ before if the function is blueprint native
